...
function parseCsv(text: string) {
  ...
  // Excel and most ERP exports write numeric dates like 12/15/2025 or
  // 12-12-2025, and one export can mix both separators in a single header row.
  // Neither regex above matches either form, so a raw export used to yield zero
  // date columns and the tool reported nothing at all.
  const numericRegex = /^\s*(\d{1,2})[-\/](\d{1,2})[-\/](\d{4})\s*$/
  ...
  fields.forEach((f, idx) => {
    ...
    if (isoRegex.test(trimmed)) {
      ...
      const n = trimmed.match(numericRegex) as RegExpMatchArray
      let month = parseInt(n[1], 10)
      let day = parseInt(n[2], 10)
      const year = parseInt(n[3], 10)
      // Read month first, which is what these exports produce, unless that is
      // impossible: 25/12/2025 can only be day first, and guessing wrong there
      // would silently place a count in the wrong month rather than fail.
      if (month > 12 && day <= 12) {
        const swap = month
        month = day
        day = swap
  ...
  if (dateHeadersWithIso.length === 0) {
    if (errorBox) errorBox.textContent = 'No date columns found. Column headers holding counts must be dates: 12/15/2025, 12-15-2025, 2025-12-15 or 15-Dec are all accepted.'
    state.series = []
    state.validation = null
    renderAnalystTable()
    renderManagementTable()
